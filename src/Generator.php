...
class Generator
{
    ...
    {
        // Extract module name from path (works with any directory structure like api/php/board/)
        preg_match('/\/([^\/]+)\/types\/Props\.php$/', $filePath, $matches);
    ...
    {
        // Check if class name contains the module name and ends with Props
        // Works with any namespace (Modules\Board\Types\, Api\Php\Board\Types\, etc.)
        $normalizedModule = ucfirst(strtolower($moduleName));
        return (
            stripos($className, '\\' . $normalizedModule . '\\') !== false &&
            str_ends_with($className, 'Props')
        );
    }
    ...
    private function generateTypesFile(): void
    {
        ...
        $content .= "// Generated from api/*/*/types/Props.php\n\n";
    ...
    /**
     * Generate routes-meta.generated.ts file (server-safe, no component imports)
     */
    private function generateRoutesMetaFile(): void
    {
        $content = "// Auto-generated by PhpSsrReact\\Generator - DO NOT EDIT\n";
        $content .= "// Server-safe route metadata (no component imports)\n\n";
        ...
        $content .= "export interface RouteMeta {\n";
        $content .= "  path: string;\n";
        $content .= "  title: string;\n";
        $content .= "  loaders?: Record<string, LoaderConfig>;\n";
        $content .= "}\n\n";
        ...
        foreach ($sortedRoutes as $route) {
            ...
            if ($loadersStr) {
                $content .= "  { path: '{$route['path']}', title: '{$route['title']}', loaders: {$loadersStr} },\n";
            } else {
                $content .= "  { path: '{$route['path']}', title: '{$route['title']}' },\n";
            }
        }
        $content .= "];\n";

        $this->writeFile($this->outputDir . '/routes-meta.generated.ts', $content);
    }
    ...
    {
        // "board/Detail.tsx" -> "../frontend/pages/board/Detail"
        $path = preg_replace('/\.tsx?$/', '', $page);
        return '../frontend/pages/' . $path;
    }
